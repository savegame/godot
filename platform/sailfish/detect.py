...
def can_build():
    ...
    if (sdl_error):
        print("pkg-config not found.. Sailfish SDL2 disabled.")
        return False
...
def get_opts():
    ...
    return [
        ...
        BoolVariable('use_static_cpp', 'Link stdc++ statically', 'no'),
        BoolVariable('use_sanitizer', 'Use LLVM compiler address sanitizer', 'no'),
        BoolVariable('use_leak_sanitizer', 'Use LLVM compiler memory leaks sanitizer (implies use_sanitizer)', 'no'),
        BoolVariable('pulseaudio', 'Detect & use pulseaudio', 'yes'),
...
def configure(env):
    ...
    if (env['builtin_libtheora'] == "yes" ):
        list_of_x86 = ['x86_64', 'x86', 'i386', 'i586']
        if any(platform.machine() in s for s in list_of_x86):
            env["x86_libtheora_opt_gcc"] = "yes"
