...
def can_build():
    ...
    sfapp_error = os.system("pkg-config sailfishapp --modversion > /dev/null ")
    if (sfapp_error):
        print("sailfishapp not found. Sailfish build disabled. Install sailfishapp-devel for all your targets in MerSDK ")
        return False
    ...
    qt_error = os.system("pkg-config Qt5Gui --modversion > /dev/null")
    if(qt_error):
         print("qt5-base-devel not found. Install qt5-devel for all your targets in MerSDK")
         return False;
...
def configure(env):
    ...
    env.Append(CPPPATH=['#platform/sailfish_qt','#core', '#thirdparty/glad'])
    ...
    env.Append(CPPFLAGS=['-DSAILFISH_FORCE_LANDSCAPE','-Di386'])
    # include paths for different versions of SailfishSDK width different SDL2 version  
    env.Append(LIBS=['dl', 'GLESv2', 'EGL', 'pthread'])
