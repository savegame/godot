...
Error OS_Sailfish::initialize(const VideoMode& p_desired, int p_video_driver, int p_audio_driver) {
	main_loop = NULL;
...
//#endif
	...
	ERR_FAIL_COND_V(!visual_server, ERR_UNCONFIGURED);
	visual_server->init();
...
void OS_Sailfish::set_custom_mouse_cursor(const RES &p_cursor, CursorShape p_shape, const Vector2 &p_hotspot)
{
	// TODO: implement
}

bool OS_Sailfish::_check_internal_feature_support(const String &p_feature) {

	return p_feature == "pc" || p_feature == "s3tc";
}

int OS_Sailfish::get_current_video_driver() const {
	return 0;
}
